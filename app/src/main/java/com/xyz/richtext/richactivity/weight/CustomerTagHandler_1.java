...
import com.xyz.richtext.CommonApp;
    ...
    private void handlerEndFONT(Editable output) {
        if (!isEmpty(propertyValue)) {
            try {
                ...
                output.setSpan(new AbsoluteSizeSpan(sp2px(CommonApp.getApplication(), value)), startIndex.pop(), output.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
